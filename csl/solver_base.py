        ...
        for key in self.global_settings:
            if key in self.local_settings:
                print(
                    key
                    + ":"
                    + " " * (max_length - len(key) + 1)
                    + f"{self.local_settings[key]}"
                    + f" (global: {self.global_settings[key]})"
                )
    ...
    **Solver states**
    ...
    def solve(self, problem, **kwargs):
        ...
        **kwargs : `dict`, optional
        ...
        ### START OF ITERATIONS ###
        for self.state_dict["iteration"] in range(
            self.state_dict["iteration"],
            self.state_dict["iteration"] + self.settings["iterations"],
        ):
            ### PRIMAL ###
            (
                primal_value,
                ...
                pointwise_slacks,
                dual_grad_norm,
            ) = self.primal_dual_update(problem)
                ...
                if self.settings["COMPUTE_TRUE_DGAP"]:
                    with torch.no_grad():
                        ...
                        for ii, slack in enumerate(constraint_slacks):
                            if problem.lambdas[ii] > 0 or (
                                problem.lambdas[ii] == 0 and slack > 0
                            ):
                                dual_grad_norm += slack.item() ** 2
                        for ii, slack in enumerate(pointwise_slacks):
                            inactive = torch.logical_or(
                                problem.mus[ii] > 0,
                    ...
                    if constraint_slacks is None and pointwise_slacks is None:
                        ...
                        if constraint_slacks is not None:
                            # L += torch.sum(torch.cat([problem.lambdas, constraint_slacks], dim=1))
                            #     [
                            #         lambda_value * slack
                            #         for lambda_value, slack in zip(
                            #             problem.lambdas, constraint_slacks
                            #         )
                            #     ]
                            # ).item()
                            L += torch.sum(
                        ...
                        if pointwise_slacks is not None:
                            L += torch.sum(
                                torch.tensor(
                                    [
                                        torch.dot(mu_value, slack)
                                        for mu_value, slack in zip(
                                            problem.mus, pointwise_slacks
                                        )
                                    ]
                                )
                            ).item()
            ...
            if self.settings["STOP_USER_DEFINED"] and self.settings[
                "STOP_USER_DEFINED"
            ](problem, self.state_dict):
    ...
    def plot(self):
        ...
        if not self.state_dict["HAS_CONSTRAINTS"]:
            ...
            if (self.state_dict["N_AVG_CONSTRAINTS"] > 0) and (
                self.state_dict["N_PTW_CONSTRAINTS"] > 0
            ):
            ...
            if self.state_dict["N_PTW_CONSTRAINTS"] > 0:
                ...
                for ii in range(self.state_dict["mus_log"].shape[1]):
                    ...
                    ax2.plot(
                        iter_idx, self.state_dict["mus_log"][:, ii, 0], color=f"C{ii}"
                    )
    ...
    def _check_stopping_criteria(self):
        ...
        # Divergence
        if (
            self._has_diverged("primal_value")
            or self._has_diverged("lagrangian_value")
            or self._has_diverged("lambdas_max")
            or self._has_diverged("mus_max")
        ):
            self.settings["logger"].error(
                f"Algorithm diverged on iteration {self.state_dict['iteration']}"
            )
            self.settings["logger"].debug(
                f"Primal = {self.state_dict.get('primal_value', 0)}"
            )
            self.settings["logger"].debug(
                f"Lagrangian = {self.state_dict.get('lagrangian_value', 0)}"
            )
            self.settings["logger"].debug(
                f"Dual variable (average) = {self.state_dict.get('lambdas_max', 0)}"
            )
            self.settings["logger"].debug(
                f"Dual variable (pointwise) = {self.state_dict.get('mus_max', 0)}"
            )
            return True
        ...
        if not self.state_dict["HAS_CONSTRAINTS"]:
            ...
            if self._is_primal_optimal():
                self.settings["logger"].info(
                    "Stopping criterion: primal absolute near-optimality"
                )
                stopping = True
        else:
            ...
            if self._is_primal_optimal() and self._is_absolutely_feasible():
                self.settings["logger"].info(
                    "Stopping criterion: primal absolute near-optimality and primal absolute feasibility"
                )
                stopping = True

            # Primal-dual absolute optimality and absolute feasibility
            if (
                self._is_primal_dual_absolutely_optimal()
                and self._is_absolutely_feasible()
            ):
                self.settings["logger"].info(
                    "Stopping criterion: primal-dual absolute near-optimality and absolute feasibility"
                )
                stopping = True

            # Primal-dual relative optimality and relative feasibility
            if (
                self._is_primal_dual_relatively_optimal()
                and self._is_relatively_feasible()
            ):
                self.settings["logger"].info(
                    "Stopping criterion: primal-dual relative near-optimality and relative feasibility"
                )
                stopping = True
        ...
        if self.settings["STOP_PATIENCE"]:
            if self._is_stalled():
                self._update_state(
                    no_update_iterations=self.state_dict["no_update_iterations"] - 1
                )
                if self.state_dict["no_update_iterations"] == 0:
                    self.settings["logger"].info(
                        "Stopping criterion: no improvement for "
    ...
    def _has_diverged(self, state):
        ...
        """
        return self.state_dict.get(state, 0) >= self.settings[
            "STOP_DIVERGENCE"
        ] or np.isnan(self.state_dict.get(state, 0))
    ...
    def _is_primal_dual_absolutely_optimal(self):
        ...
        if self.settings["STOP_ABS_DGAP"]:
            truth_value = (
                self.state_dict["duality_gap"] < self.settings["STOP_ABS_DGAP"]
            )
        elif self.settings["STOP_PGRAD"] and self.settings["STOP_DGRAD"]:
            truth_value = (
                self.state_dict["primal_grad_norm"] < self.settings["STOP_PGRAD"]
            ) and (self.state_dict["dual_grad_norm"] < self.settings["STOP_DGRAD"])
        else:
            truth_value = False
    ...
    def _is_absolutely_feasible(self):
        ...
        if self.settings["STOP_ABS_FEAS"] or self.settings["STOP_NFEAS"]:
            ...
            if self.settings["STOP_ABS_FEAS"]:
                truth_value = truth_value and (
                    np.max(self.state_dict.get("constraint_feas", 0))
                    < self.settings["STOP_ABS_FEAS"]
                )

            if self.settings["STOP_NFEAS"]:
                truth_value = truth_value and (
                    np.max(self.state_dict.get("pointwise_nfeas", 1))
                    >= self.settings["STOP_NFEAS"]
                )
        else:
            truth_value = False
    ...
    def _is_relatively_feasible(self):
        ...
        if self.settings["STOP_REL_FEAS"] or self.settings["STOP_NFEAS"]:
            ...
            if self.settings["STOP_REL_FEAS"]:
                truth_value = truth_value and (
                    np.max(self.state_dict.get("constraint_rel_feas", 0))
                    < self.settings["STOP_REL_FEAS"]
                )
    ...
    def _is_stalled(self):
        ...
        """
        truth_value = (
            self.state_dict["primal_value"] >= self.state_dict["primal_value_prev"]
        )

        if self.state_dict["N_AVG_CONSTRAINTS"] > 0:
            truth_value = truth_value or np.all(
                np.clip(self.state_dict["constraint_feas"], 0, None)
        ...
        if self.state_dict["N_PTW_CONSTRAINTS"] > 0:
            truth_value = truth_value or np.all(
                ...
                <= self.state_dict["pointwise_nfeas_prev"]
            )
    ...
    def _initialize(self, problem):
        ...
        self.state_dict["HAS_CONSTRAINTS"] = (
            self.state_dict["N_AVG_CONSTRAINTS"] > 0
        ) or (self.state_dict["N_PTW_CONSTRAINTS"] > 0)
        ...
        if self.primal_solver is None:
            self.primal_solver = self.settings["primal_solver"](
        ...
        if self.dual_solver is None and (problem.lambdas or problem.mus):
            self.dual_solver = self.settings["dual_solver"](
                problem.lambdas + problem.mus
            )
        # Load previous settings, if they exist
    ...
    def _initialize_logs(self):
        ...
        if (
            self.state_dict["HAS_CONSTRAINTS"]
            and "lagrangian_value_log" not in self.state_dict
        ):
            self.state_dict["lagrangian_value_log"] = np.zeros(0)

        if self.state_dict["N_AVG_CONSTRAINTS"] > 0:
            if "feas_log" not in self.state_dict:
                self.state_dict["feas_log"] = np.zeros(
                    [0, self.state_dict["N_AVG_CONSTRAINTS"]]
                )
            if "rel_feas_log" not in self.state_dict:
                self.state_dict["rel_feas_log"] = np.zeros(
                    [0, self.state_dict["N_AVG_CONSTRAINTS"]]
                )
            if "lambdas_log" not in self.state_dict:
                self.state_dict["lambdas_log"] = np.zeros(
                    [0, self.state_dict["N_AVG_CONSTRAINTS"]]
                )

        if self.state_dict["N_PTW_CONSTRAINTS"] > 0:
            if "nfeas_log" not in self.state_dict:
                self.state_dict["nfeas_log"] = np.zeros(
                    [0, self.state_dict["N_PTW_CONSTRAINTS"]]
                )
            if "mus_log" not in self.state_dict:
                self.state_dict["mus_log"] = np.zeros(
                    [0, self.state_dict["N_PTW_CONSTRAINTS"], 3]
                )
    ...
    def _print_log(self, logger, level=logging.INFO, header=False):
        ...
        if self.state_dict["HAS_CONSTRAINTS"]:
            message = {
                "Iteration": f"{self.state_dict.get('iteration', 0) + 1}",
            ...
            if self.state_dict["N_AVG_CONSTRAINTS"] > 0:
                message[
                    "FEAS_AVG"
                ] = f"{np.max(self.state_dict.get('constraint_feas', 0)):.3g}"
                message[
                    "RFEAS_AVG"
                ] = f"{np.max(self.state_dict.get('constraint_rel_feas', 0)):.3g}"

            if self.state_dict["N_PTW_CONSTRAINTS"] > 0:
                message[
                    "FEAS_PTW"
                ] = f"{np.max(self.state_dict.get('pointwise_feas', 0)):.3g}"
                message[
                    "NFEAS_PTW"
                ] = f"{np.max(self.state_dict.get('pointwise_nfeas', 0)):.3g}"
        else:
            message = {
                "Iteration": f"{self.state_dict.get('iteration', 0) + 1}",
                "P": f"{self.state_dict.get('primal_value', 0):.3g}",
        ...
        if header:
            logger.log(
                level,
                ("{:<9} | " * (len(message) - 1) + "{:<9}").format(*message.keys()),
            )
        else:
            logger.log(
                level,
                ("{:<9} | " * (len(message) - 1) + "{:<9}").format(*message.values()),
            )
    ...
    def _log_primal(self, primal_value, primal_grad_norm):
        ...
        self._update_state(primal_value=primal_value, primal_grad_norm=primal_grad_norm)

        ### Update trace ###
        self.state_dict["primal_value_log"] = np.append(
            self.state_dict["primal_value_log"], self.state_dict["primal_value"]
        )
    ...
    def _log_dual(
        ...
        # Dual value
        self._update_state(
            lagrangian_value=lagrangian_value, dual_grad_norm=dual_grad_norm
        )
        self._update_state(
            duality_gap=np.abs(
                self.state_dict["primal_value"] - self.state_dict["lagrangian_value"]
            )
        )
        ...
        if np.abs(self.state_dict["primal_value"]) < 1e-6:
            ...
            self._update_state(
                rel_duality_gap=self.state_dict["duality_gap"]
        ...
        if self.state_dict["N_AVG_CONSTRAINTS"] > 0 and constraint_slacks is not None:
            self._update_state(
                constraint_feas=np.array(
                    [slacks.detach().to("cpu") for slacks in constraint_slacks], ndmin=2
                )
            )
            self._update_state(
                constraint_rel_feas=np.array(
                    [
                        (slack.detach() / c).to("cpu") if (c != 0) else float("Inf")
                        for slack, c in zip(constraint_slacks, rhs)
                    ],
                    ndmin=2,
                )
            )
            self._update_state(
                constraint_nfeas=np.mean(
                    self.state_dict.get("constraint_feas") <= 0, axis=1
                )
            )
        ...
        if self.state_dict["N_PTW_CONSTRAINTS"] > 0 and pointwise_slacks is not None:
            self._update_state(
                pointwise_feas=np.concatenate(
                    [slacks.detach().to("cpu") for slacks in pointwise_slacks]
                )
            )
            self._update_state(
                pointwise_rel_feas=np.concatenate(
                    [
                        (slack.detach() / c).to("cpu")
                        if torch.all(c != 0)
                        else float("Inf")
            ...
            self._update_state(
                pointwise_nfeas=np.array(
                    [
                        np.mean(value.detach().to("cpu").numpy() <= 0)
                        for value in pointwise_slacks
                    ],
                    ndmin=2,
                )
            )
        ...
        ### Update traces ###
        # Dual value
        self.state_dict["lagrangian_value_log"] = np.append(
            self.state_dict["lagrangian_value_log"], self.state_dict["lagrangian_value"]
        )

        # Average constraint feasibility
        if self.state_dict["N_AVG_CONSTRAINTS"] > 0:
            self.state_dict["feas_log"] = np.append(
                self.state_dict["feas_log"],
                self.state_dict.get("constraint_feas", 0),
                axis=0,
            )
            self.state_dict["rel_feas_log"] = np.append(
                self.state_dict["rel_feas_log"],
                self.state_dict.get("constraint_rel_feas", 0),
                axis=0,
            )
            self.state_dict["lambdas_log"] = np.append(
                self.state_dict["lambdas_log"],
                np.array([lambda_value.to("cpu") for lambda_value in lambdas], ndmin=2),
                axis=0,
            )
            self._update_state(
                lambdas_max=np.max(self.state_dict["lambdas_log"][-1, :])
            )

        # Pointwise constraint feasibility
        if self.state_dict["N_PTW_CONSTRAINTS"] > 0:
            self.state_dict["nfeas_log"] = np.append(
                self.state_dict["nfeas_log"], self.state_dict["pointwise_nfeas"], axis=0
            )
            self.state_dict["mus_log"] = np.append(
                self.state_dict["mus_log"],
                np.array(
                    [
                        [
                            mu_value.to("cpu").numpy().mean(),
